...
<figure class="diagram-card">
  <img src="/assets/img/rdesk-bridge.svg" alt="Ralph Loop의 인사이트가 rlp-desk 구현으로 이어지는 흐름 다이어그램">
  <figcaption>이 글은 Ralph Loop의 개념이 실제 구현으로 어떻게 내려오는지 보여주는 허브 역할을 한다.</figcaption>
</figure>
...
Codex는 `Agent()`가 아니라 subprocess로 호출한다. 현재 `rlp-desk`의 구현과 문서를 보면 `codex exec`가 핵심이다.
...
최근 `rlp-desk`의 이력과 테스트 문서를 보면, 이 구조는 한 단계 더 구체화돼 있다.
...
<section class="series-card">
  ...
  <ol class="series-list">
    ...
    <li><strong>지금 글:</strong> fresh context를 rlp-desk는 어떻게 구현하나</li>
